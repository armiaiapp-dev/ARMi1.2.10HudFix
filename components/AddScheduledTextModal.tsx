...
interface AddScheduledTextModalProps {
  visible: boolean;
  onClose: () => void;
  onTextScheduled: () => void;
  theme: any;
}
...
export function AddScheduledTextModal({ visible, onClose, onTextScheduled, theme }: AddScheduledTextModalProps) {
  ...
  const loadProfiles = async () => {
    try {
      console.log('🔍 DEBUG: Loading profiles for scheduled text modal...');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error('Error creating scheduled text:', error);
      Alert.alert('Error', 'Failed to schedule text');
    } finally {
      setLoading(false);
    }
  };
      ...
      >
        <View style={[styles.modal, { backgroundColor: theme.cardBackground }]}>
          ...
          <View style={[styles.header, { borderBottomColor: theme.border }]}>
            <Text style={[styles.title, { color: theme.text }]}>Schedule Text</Text>
          ...
          <View style={[styles.footer, { borderTopColor: theme.border }]}>
            ...
            >
              <Text style={styles.saveButtonText}>
                {loading ? 'Scheduling...' : 'Schedule Text'}
              </Text>
            </TouchableOpacity>
          </View>
        </View>
